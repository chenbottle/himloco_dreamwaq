...
class Go2_legstand(BaseTask):
    ...
    def post_physics_step(self):
        ...
        self.projected_forward_vec[:] = quat_rotate(self.base_quat, self.forward_vec)
    ...
    def reset_idx(self, env_ids):
        ...
        # 生成等长的随机 0/1 掩码，50 % 概率
        mask = torch.randint(0, 2, (len(env_ids),), device=self.stand_command.device)

        # 写回
        self.stand_command[env_ids] = mask
        # fill extras
        self.extras["episode"] = {}
        ...
        self.extras["episode"]["robot_base_height"] = torch.mean(self.root_states[env_ids, 2])
    ...
    def compute_observations(self):
        ...
        self.obs_buf = torch.cat((  
                    ...
                    self.actions,#12 动作
                    self.stand_command.unsqueeze(1)
                    ),dim=-1)
    ...
    def _resample_commands(self, env_ids):
        ...
        # ---------- 3. 小概率全归零（保留你原来的逻辑） ----------
        all_zero_mask = torch.rand(len(env_ids), device=self.device) < 0.10
    ...
    def _compute_torques(self, actions):
        ...
        act=(actions + self.motor_zero_offsets)
    ...
    def _get_noise_scale_vec(self):
        ...
        noise_vec[3:6] = noise_scales.gravity         # euler x,y
    ...
    def _init_buffers(self):
        ...
        self.contact_forces = gymtorch.wrap_tensor(net_contact_forces).view(self.num_envs, -1, 3) # shape: num_envs, num_bodies, xyz axis
        self.stand_command=torch.zeros(self.num_envs, dtype=torch.int64, device=self.device)
        self.feet_pos = self.rigid_state.view(self.num_envs, self.num_bodies, 13)[:, self.feet_name_reward_indices, 0:3]
        self.feet_contact_pos = self.rigid_state.view(self.num_envs, self.num_bodies, 13)[:, self.contact_foot_indices, 0:3]
        self.feet_contact_vel = self.rigid_state.view(self.num_envs, self.num_bodies, 13)[:, self.contact_foot_indices, 7:10]
    ...
    def _reward_base_height(self):
        ...
        return torch.square(base_height - self.cfg.rewards.base_height_target)*(self.stand_command==0)

    def _reward_base_height_stand(self):
        # Penalize base height away from target
        base_height = self._get_base_heights()
        return torch.exp(-torch.abs(base_height - self.cfg.rewards.base_height_target_stand)*5)*(self.stand_command==1)
    ...
    def _reward_action_rate_2(self):
        # Penalize changes in actions
    ...
    def _reward_tracking_lin_vel(self):
        ...
        # print(self.commands[0, :2],self.base_lin_vel[0, :2],self.root_states[0, 7:9])
    ...
    def _reward_joint_pos(self):
        dof_scale = torch.tensor([3.0, 0, 0., 3.0, 0., 0., 3.0, 0., 0., 3.0, 0., 0.,], device=self.device, requires_grad=False,)
        err = (self.dof_pos - self.default_dof_pos) *dof_scale
        return torch.exp(-torch.sum(torch.square(err), dim=1))
    ...
    def _reward_still(self):
        ...
        return torch.sum(torch.abs(dof_err), dim=1) *(self.stand_command==0)
    
    def _reward_still_stand(self):
        # Penalize motion at running commands        
        dof_err = self.dof_pos - self.descire_joint_pos
        return torch.sum(torch.abs(dof_err), dim=1) *(self.stand_command==1)*(self.rew_hanstand>0.5)

    def _reward_default_pos_reward(self):
        # Penalize motion at zero commands
        return torch.exp(-torch.sum(torch.abs(self.dof_pos - self.descire_joint_pos)[:,6:], dim=1))*(self.stand_command==1)*(self.rew_hanstand>0.5)
    ...
    def _reward_orientation(self):
        return torch.square((self.projected_gravity - self.gravity_vec)).sum(dim=1)*(self.stand_command==0)

    def _reward_handstand_orientation(self):
        return torch.square((self.projected_gravity - self.target_gravity)).sum(dim=1)*(self.stand_command==1)
    ...
    def _reward_handstand(self):
        # 目标向量
        target_vec = torch.tensor([0, 0, 1], device=self.device).unsqueeze(0).repeat(self.num_envs, 1)
        forward_error = torch.sum(self.projected_forward_vec * target_vec, dim=-1)
        # print("stand_command == 1", self.stand_command == 1)
        return forward_error * (self.stand_command == 1)

    def _reward_forward_contact(self):
        contact = self.contact_forces[:, self.feet_indices, 2] > 5.
        contact_filt = torch.logical_or(contact, self.last_contacts_2)
        self.last_contacts_2 = contact

        return torch.sum(contact_filt[:, [0,1]], dim=1) * (self.stand_command == 1)
    ...
    def _reward_symmetric_joints(self):
        ...
        dof[:,1,0] *= -1
        dof[:,3,0] *= -1
        err = torch.sum(torch.abs(dof[:,2,:] - dof[:,3,:]),axis=1) + 0.5*torch.sum(torch.abs(dof[:,0,:] - dof[:,1,:]),axis=1)
        return err*(self.stand_command==1)
