...
from legged_gym.envs.D1_W.d1_w_config import D1_W_Cfg_Yu

class D1_W_Robot(BaseTask):
    def __init__(self, cfg: D1_W_Cfg_Yu, sim_params, physics_engine, sim_device, headless):
    ...
    def _reward_stand_still_wheel(self):
        # Penalize wheel motion when commands are zero (standing still)
        # Only consider wheel joints for this reward
        wheel_vel = self.dof_vel[:, self.wheel_indices]
        return torch.sum(torch.abs(wheel_vel), dim=1) * (torch.norm(self.commands[:, :2], dim=1) < 0.1)

    def _reward_feet_contact_forces(self):
        # penalize high contact forces
        return torch.sum((torch.norm(self.contact_forces[:, self.feet_indices, :], dim=-1) -  self.cfg.rewards.max_contact_force).clip(min=0.), dim=1)

    def _reward_wheels_contact(self):
        # Encourage all wheels to be in contact with ground when standing still
        # Check if robot is in standing mode (commands near zero)
        is_standing = (torch.norm(self.commands[:, :2], dim=1) < 0.1).float()
        wheel_contact_forces = self.contact_forces[:, self.feet_indices, 2]  # Z轴接触力
        wheel_contact = (wheel_contact_forces > self.cfg.rewards.min_wheel_contact_force).float()
        missing_contacts = 4 - torch.sum(wheel_contact, dim=1)
        return missing_contacts * is_standing
